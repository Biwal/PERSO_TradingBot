...
class BtcTradingGraph:
    ...
    def _render_price(self, current_step, net_worth, dates, step_range):
        ...
        candlestick(
            self.price_ax, candlesticks, width=1, colorup=UP_COLOR, colordown=DOWN_COLOR
        )
    ...
    def _render_volume(self, current_step, net_worth, dates, step_range):
        ...
        pos = (
            self.df["Open"].values[step_range] - self.df["Close"].values[step_range] < 0
        )
        neg = (
            self.df["Open"].values[step_range] - self.df["Close"].values[step_range] > 0
        )
        ...
        self.volume_ax.bar(
            dates[pos], volume[pos], color=UP_COLOR, alpha=0.4, width=1, align="center"
        )
        self.volume_ax.bar(
            dates[neg],
            volume[neg],
    ...
    def render(self, current_step, net_worth, trades, window_size=40):
        ...
        self.price_ax.set_xticklabels(
            self.df["Timestamp"].values[step_range],
            rotation=45,
